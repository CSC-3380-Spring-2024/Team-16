...
public class Quantity {
    public static double dryToGram(double quantity, String unit) {
        switch (unit.toLowerCase()) {
            ...
            case "unit(regular)":
                return quantity * IngredientRelated.Ingredient.gram_per_unit; // 1 unit * grams per units = grams
            case "unit(baby)":
                return quantity * IngredientRelated.Ingredient.gram_per_unit / 5;
            default:
                throw new IllegalArgumentException("Invalid unit: " + unit);
        }
    }

    public static double inverseDryToGram(double grams, String unit) {
        switch (unit.toLowerCase()) {
            case "kg":
                return grams / 1000;
            case "g":
            ...
            case "unit(regular)":
                return grams / IngredientRelated.Ingredient.gram_per_unit; // grams / grams per unit = units
            case "unit(baby)":
                return grams / (IngredientRelated.Ingredient.gram_per_unit / 5);
            default:
                throw new IllegalArgumentException("Invalid unit: " + unit);
        }
    }
